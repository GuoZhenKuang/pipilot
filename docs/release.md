...
Plan 013 release notes: the Sessions cockpit is cache-first across hosts with a maximum of two concurrent index refreshes. Saved pin/hidden state contains only local stable keys and density; deleted profiles clear their local scope, while unavailable saved sessions remain removable/retryable placeholders. Selected workspace context is host-profile-scoped, and normal cards/search omit full cwd and session paths. Quick reply is text-only, uses existing controller locks plus an expected-active-key dispatch guard, and never switches away from another active run or navigates after send unless explicitly requested. Device evidence remains **PENDING — operator-owned**.
