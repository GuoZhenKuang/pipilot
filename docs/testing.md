...
Plan 013's non-device tests cover key-only pin/hidden persistence and corruption; profile add/edit/delete lifecycle; host-scoped cwd selection across all-host filtering, host switches and cross-host resume; privacy-safe projection/search; cache-first two-host-bounded refresh with partial failures; deterministic ordering; and quick-reply active/idle/error/cancellation/duplicate/null-identity/concurrent-switch branches. Cockpit Compose tests cover empty and hidden-recovery states, filters, host staleness and default-card path privacy; they are compiled but remain operator-owned for device execution. Device acceptance remains operator-owned. When enabled, verify compact/expanded layout, large fonts, TalkBack labels/focus, keyboard/IME quick reply, hidden recovery, host add/delete, one unreachable host, and lock conflicts without takeover or automatic navigation.
